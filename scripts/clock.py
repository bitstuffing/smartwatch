...
class Clock():
    ...
    def getTickPosition(self, tick=0, radio=32, originx=64, originy=32):
        ...
        y = -1 * math.sin(2 * math.pi * (tick / 60.0)) # "-1 *" the y coordinates increase going down (the opposite of how they normally go in mathematics)
    ...
    def displayClock(self,screen,hours=0,minutes=0,seconds=0,ratio=32,originx=64,originy=32):
        hours = hours % 12
        x, y = self.getTickPosition(tick=int(hours*5),radio=int(ratio/2),originx=originx,originy=originy)
